...
export interface DashboardWidget {
  ...
  permission?: string; // Required permission key e.g. "accounting.view"
  order: number;
  colSpanDesktop: string; // Tailwind grid span e.g. "col-span-12", "lg:col-span-4", "lg:col-span-3"
  render: (props: DashboardWidgetContextProps) => React.ReactNode;
}
...
export const DASHBOARD_WIDGET_REGISTRY: DashboardWidget[] = [
  ...
  {
    id: "kpi-metrics-row",
    title: "KPI Metrics Summary",
    category: "kpi",
    permission: "bookings.view", // Also checks sub-card permissions internally
    order: 10,
    colSpanDesktop: "col-span-12",
    render: ({ stats, loading, navigate, hasPermission, userPerms, userRole }) => {
      const showRevenue = hasPermission(userPerms, "accounting.view", userRole);
      const showPendingCustomers = hasPermission(userPerms, "accounting.view", userRole) || hasPermission(userPerms, "bookings.view", userRole);
      const showPendingVendors = hasPermission(userPerms, "accounting.view", userRole) || hasPermission(userPerms, "vendors.view", userRole);
      const showTripsRunning = hasPermission(userPerms, "trips.view", userRole) || hasPermission(userPerms, "ops.view", userRole);
      const showBookingsMonth = hasPermission(userPerms, "bookings.view", userRole);
          ...
          {showRevenue && (
            ...
            >
              <div className="flex items-start justify-between">
                <span className="text-[10px] font-bold text-[#74839A] uppercase tracking-[0.4px]">Revenue</span>
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[9.5px] font-semibold text-emerald-600 flex items-center gap-0.5 mt-1">
                  ▲ Total <span className="text-[#74839A] font-medium">collections</span>
                </p>
              </div>
            </div>
          )}
  ...
  // ─── 2. NEEDS YOUR ATTENTION WIDGET ───
  {
    ...
    order: 20,
    colSpanDesktop: "col-span-1 lg:col-span-4",
    render: ({ stats, navigate }) => (
  ...
  // ─── 4. TRIPS DEPARTING NEXT 7 DAYS WIDGET ───
  {
  ...
  // ─── 5. TODAY'S SCHEDULE WIDGET ───
  {
    ...
    permission: "ops.view",
    order: 50,
    colSpanDesktop: "col-span-1 sm:col-span-1 lg:col-span-3",
    render: ({ stats, navigate }) => (
      <div className="bg-white border border-[#E3EAF2] rounded-[10px] shadow-[0_1px_2px_rgba(15,23,42,0.04)] flex flex-col overflow-hidden h-full">
        <div className="h-9 px-3.5 flex items-center justify-between border-b border-[#E3EAF2] shrink-0">
  ...
  // ─── 6. MY APPROVAL QUEUE WIDGET ───
  {
    ...
    order: 60,
    colSpanDesktop: "col-span-1 sm:col-span-1 lg:col-span-3",
    render: ({ ticketPendingCount, navigate }) => (
      <div className="bg-white border border-[#E3EAF2] rounded-[10px] shadow-[0_1px_2px_rgba(15,23,42,0.04)] flex flex-col overflow-hidden h-full">
        <div className="h-9 px-3.5 flex items-center justify-between border-b border-[#E3EAF2] shrink-0">
  ...
  // ─── 8. ANNOUNCEMENTS WIDGET ───
  {
    ...
    order: 80,
    colSpanDesktop: "col-span-1 sm:col-span-1 lg:col-span-3",
    render: ({ announcements, loadingAnnouncements, setShowAddAnnouncement, setShowAllAnnouncements, hasPermission, userPerms, userRole }) => (
      <div className="bg-white border border-[#E3EAF2] rounded-[10px] shadow-[0_1px_2px_rgba(15,23,42,0.04)] flex flex-col overflow-hidden h-full">
        <div className="h-9 px-3.5 flex items-center justify-between border-b border-[#E3EAF2] shrink-0">
