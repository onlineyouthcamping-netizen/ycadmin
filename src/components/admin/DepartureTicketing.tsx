...
export default function DepartureTicketing({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const result = await trainTicketService.getEffectiveTemplates(
        tripId,
        departureDateStr,
      );
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
        <div className="mb-4">
          <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
            <Ticket className="w-4 h-4 text-purple-600" /> Applicable Ticketing
            Templates
          </h3>
          <p className="text-xs text-slate-500 mt-1">
            Global (TRIP) and Override (DEPARTURE) templates active for this
            specific date.
          </p>
        </div>
        <TicketTemplateList
