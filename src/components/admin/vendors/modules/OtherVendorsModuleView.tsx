...
import { Building2, MapPin, Phone, Mail, Plus, RotateCw, Eye, Pencil } from "lucide-react";
...
export function OtherVendorsModuleView({
  ...
  const otherVendors = vendors.filter(v => !["HOTEL", "RESORT", "HOMESTAY", "HOSTEL", "TRANSPORT", "ACTIVITIES", "RESTAURANT", "GUIDE", "CAMPING"].includes(v.type));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight">Other Vendor Partners Subsystem</h1>
          </div>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">Misc Local Vendors, Equipment Suppliers & Special Services</p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100 font-medium">
            {otherVendors.map((v) => (
              <tr key={v.id} className="hover:bg-slate-50/80 transition-colors">
                <td className="p-3.5 font-extrabold text-slate-800">{v.name}</td>
                <td className="p-3.5">
                  <span className="bg-slate-100 text-slate-700 font-bold px-2 py-0.5 rounded text-[10px] uppercase border border-slate-200">
                    {v.type}
                  </span>
                </td>
                <td className="p-3.5 text-slate-700 font-bold">{v.contactPerson || v.phone || "—"}</td>
                <td className="p-3.5 font-bold text-slate-800">{v.city || v.location || "—"}</td>
                <td className="p-3.5 text-slate-700">{v.paymentTerms || "30 Days Credit"}</td>
                <td className="p-3.5 text-right space-x-1.5">
                  <Button onClick={() => onSelectVendor(v)} className="h-8 text-xs bg-[#F97316] hover:bg-[#E05E00] text-white font-bold px-2.5 rounded-lg">
                    <Eye className="w-3.5 h-3.5 mr-1" /> View Workspace
                  </Button>
                  <button onClick={() => onEditVendor(v)} className="p-1.5 text-slate-400 hover:text-slate-700 bg-slate-100 rounded-md">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 text-xs text-slate-600 font-medium">
        <span>Showing {pagination.startIndex}–{pagination.endIndex} of {pagination.total} vendor partners</span>
