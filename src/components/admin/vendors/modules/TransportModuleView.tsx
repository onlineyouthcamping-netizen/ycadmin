...
export function TransportModuleView({
  ...
  const kpis = [
    {
      title: "Fleet Operator Firms",
      count: fleet.length || 8,
      icon: Bus,
      color: "text-emerald-600 bg-emerald-50 border-emerald-200",
    },
    {
      title: "Tempo Travellers (12-26S)",
      count: 24,
      icon: ShieldCheck,
      color: "text-blue-600 bg-blue-50 border-blue-200",
    },
    {
      title: "SUVs & Urban Cruisers",
      count: 16,
      icon: UserCheck,
      color: "text-purple-600 bg-purple-50 border-purple-200",
    },
    {
      title: "Valid All-India Permits",
      count: "100%",
      icon: FileText,
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight">
              Transport & Fleet Management Subsystem
            </h1>
          </div>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">
            Tempo Travellers, Buses, SUVs, Fleet Permits & Drivers
          </p>
        </div>
        <Button
          onClick={onAddVendor}
        ...
        <div className="lg:col-span-3 space-y-4">
                ...
                {fleet.map((f) => (
                  ...
                  >
                    ...
                    <td className="p-3.5 font-bold text-slate-800 whitespace-nowrap">
                      {f.seatCapacity
                        ? `${f.seatCapacity}-Seater ${f.fleetType || "Tempo Traveller"}`
                        : f.fleetType || "17-Seater Tempo Traveller"}
                    </td>
                    <td className="p-3.5 text-slate-700 font-bold whitespace-nowrap">
                      {f.contactPerson || "Suresh Rathod"} (
                      {f.phone || "+91 98166 00000"})
                    </td>
                    ...
                    <td className="p-3.5 text-right font-black text-slate-800 whitespace-nowrap">
                      ₹4,500 / Day
                    </td>
          ...
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 text-xs text-slate-600 font-medium">
            <span>
              ...
              {pagination.total} fleet operators
            </span>
        ...
        <div className="space-y-4">
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs space-y-3 text-xs">
            <h3 className="font-extrabold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <ShieldAlert className="w-4 h-4 text-emerald-600" /> Fleet
              Compliance Radar
            </h3>
            <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-900 font-medium space-y-1">
              <span className="font-bold block">
                Vehicle Permits Up to Date
              </span>
              <p>
                100% of active vehicles have valid AITP permits for Delhi-Manali
                routes.
              </p>
            </div>
          </div>
