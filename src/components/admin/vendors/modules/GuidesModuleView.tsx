...
import { UserCheck, ShieldCheck, MapPin, Plus, RotateCw, Eye, Pencil, Award, Star } from "lucide-react";
...
export function GuidesModuleView({
  ...
  const kpis = [
    { title: "NIM / IMF Certified Guides", count: guides.length || 15, icon: Award, color: "text-indigo-600 bg-indigo-50 border-indigo-200" },
    { title: "Senior Trek Leaders", count: 8, icon: UserCheck, color: "text-emerald-600 bg-emerald-50 border-emerald-200" },
    { title: "First Aid & CPR Certified", count: "100%", icon: ShieldCheck, color: "text-green-600 bg-green-50 border-green-200" },
    { title: "Average Guide Rating", count: "4.95 ★", icon: Star, color: "text-amber-600 bg-amber-50 border-amber-200" },
  ];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight">Guide & Expedition Leader Subsystem</h1>
          </div>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">Mountain Guides, Tour Leaders, Trek Experts & Certifications</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 space-y-4">
                  ...
                  <tr key={g.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="p-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="font-extrabold text-slate-800 text-sm block">{g.name}</span>
                          <span className="text-[10px] font-bold text-slate-400 font-mono uppercase">{g.vendorCode || g.id}</span>
                        </div>
                      </div>
                    </td>
                    <td className="p-3.5 font-bold text-slate-800">English, Hindi, Pahadi</td>
                    <td className="p-3.5">
                    ...
                    <td className="p-3.5 text-slate-700 font-bold">High Altitude Treks (Hampta / Pin Parvati)</td>
                    <td className="p-3.5 text-center font-black text-amber-600">4.95 ★</td>
                    <td className="p-3.5 text-right space-x-1.5">
                      <Button onClick={() => onSelectVendor(g)} className="h-8 text-xs bg-[#F97316] hover:bg-[#E05E00] text-white font-bold px-2.5 rounded-lg">
                        <Eye className="w-3.5 h-3.5 mr-1" /> View Workspace
                      </Button>
                      <button onClick={() => onEditVendor(g)} className="p-1.5 text-slate-400 hover:text-slate-700 bg-slate-100 rounded-md">
          ...
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 text-xs text-slate-600 font-medium">
            <span>Showing {pagination.startIndex}–{pagination.endIndex} of {pagination.total} guides</span>
        ...
        <div className="space-y-4">
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs space-y-3 text-xs">
            <h3 className="font-extrabold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <Award className="w-4 h-4 text-indigo-600" /> Guide Roster & Availability
            </h3>
            <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-lg text-indigo-900 font-medium space-y-1">
              <span className="font-bold block">Assigned to Upcoming Departures</span>
              <p>8 Senior leaders allocated for June 05 & June 12 Himalayan departures.</p>
            </div>
          </div>
        </div>
      </div>
