...
import { MapPin, Phone, ShieldCheck, Plus, RotateCw, Eye, Pencil } from "lucide-react";
...
export function CampingModuleView({
  ...
  const kpis = [
    { title: "Campsite Partners", count: camps.length || 9, icon: MapPin, color: "text-pink-600 bg-pink-50 border-pink-200" },
    { title: "Dome & Alpine Tents", count: 45, icon: MapPin, color: "text-amber-600 bg-amber-50 border-amber-200" },
    { title: "Swiss Luxury Tents", count: 18, icon: ShieldCheck, color: "text-emerald-600 bg-emerald-50 border-emerald-200" },
    { title: "Bonfire & Music Permitted", count: "100%", icon: MapPin, color: "text-purple-600 bg-purple-50 border-purple-200" },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tight">Camping Partners Subsystem</h1>
          </div>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">Campsite Owners, Swiss Luxury Tents & Outdoor Camping Equipment</p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100 font-medium">
            {camps.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50/80 transition-colors">
                ...
                <td className="p-3.5 font-bold text-slate-800">{c.city || c.location || "Kasol Riverside"}</td>
                <td className="p-3.5 text-slate-700 font-bold">25 Swiss Tents (Triple Sharing)</td>
                <td className="p-3.5 font-bold text-emerald-600">Attached Western Washrooms</td>
                <td className="p-3.5 text-slate-700">Bonfire, Music System, River Stream View</td>
                <td className="p-3.5 text-right font-black text-slate-800">₹1,200 / Night</td>
                <td className="p-3.5 text-right space-x-1.5">
                  <Button onClick={() => onSelectVendor(c)} className="h-8 text-xs bg-[#F97316] hover:bg-[#E05E00] text-white font-bold px-2.5 rounded-lg">
                    <Eye className="w-3.5 h-3.5 mr-1" /> View Workspace
                  </Button>
                  <button onClick={() => onEditVendor(c)} className="p-1.5 text-slate-400 hover:text-slate-700 bg-slate-100 rounded-md">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 text-xs text-slate-600 font-medium">
        <span>Showing {pagination.startIndex}–{pagination.endIndex} of {pagination.total} campsite partners</span>
