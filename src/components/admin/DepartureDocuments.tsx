        ...
        {[
          {
            ...
            count: docs.filter((d) => d.category === "PASSENGER").length,
            desc: "Aadhaar, Passports",
          },
          {
            key: "PAYMENT_PROOF",
            label: "Payment Proofs",
            count: docs.filter((d) => d.category === "PAYMENT_PROOF").length,
            desc: "Receipts, Screenshots",
          },
          {
            key: "HOTEL_VOUCHER",
            label: "Hotel Vouchers",
            count: docs.filter((d) => d.category === "HOTEL_VOUCHER").length,
            desc: "Booking Confirmations",
          },
          {
            key: "VEHICLE",
            label: "Vehicle Documents",
            count: docs.filter((d) => d.category === "VEHICLE").length,
            desc: "RC, Permits, Insurance",
          },
          {
            key: "GUIDE_ID",
            label: "Guide IDs",
            count: docs.filter((d) => d.category === "GUIDE_ID").length,
            desc: "License, Badge, Aadhaar",
          },
          {
            key: "OPERATIONAL",
            label: "Operational Files",
            count: docs.filter((d) => d.category === "OPERATIONAL").length,
            desc: "Checklists, SOPs, Lists",
          },
        ].map((tile) => (
          <button
            key={tile.key}
            onClick={() =>
              setCategoryFilter(
                categoryFilter === tile.key ? "All Categories" : tile.key,
              )
            }
            className={cn(
            ...
            {loading ? (
              ...
              filteredDocs.map((d) => (
                ...
                >
                  ...
                  <td className="p-3 border-r border-slate-100 font-semibold text-slate-500">
                  ...
                  <td className="p-3 border-r border-slate-100 text-slate-650 font-medium max-w-[200px] truncate">
                    {d.remarks || "—"}
                  </td>
      ...
      <Dialog open={uploadModalOpen} onOpenChange={setUploadModalOpen}>
        <DialogContent className="max-w-md bg-white p-5 rounded-lg border border-slate-200">
          ...
          <DialogDescription className="text-[11px] text-slate-450 mt-1">
            Specify the document type, name, storage path URL, and description
            details.
          </DialogDescription>
