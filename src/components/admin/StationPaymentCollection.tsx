...
export default function StationPaymentCollection({ tripId, departureDateStr }: Props) {
  ...
  const toggleExpand = (id: string) => {
    setExpandedBookings(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  };
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.length === 0 ? (
                ...
                <React.Fragment key={bk.id}>
                  <tr className="hover:bg-slate-50/60 transition-colors cursor-pointer" onClick={() => toggleExpand(bk.id)}>
                    ...
                    <td className="px-3 py-2.5 text-slate-500">
                      <div className="flex items-center gap-1.5">
                        <span>{bk.phone || "—"}</span>
                        {bk.phone && (
                          <a href={`tel:${bk.phone}`} className="text-blue-500 hover:text-blue-600 p-1 hover:bg-blue-50 rounded-full transition-colors" title="Call">
                  ...
                  {expandedBookings.includes(bk.id) && bk.members && bk.members.length > 0 && (
                    <tr className="bg-slate-50/80 border-b border-slate-100">
                      <td colSpan={11} className="px-4 py-3">
                        <div className="flex flex-wrap gap-3">
                          {bk.members.map((m, i) => (
                            <div key={i} className="flex items-center gap-3 bg-white px-3 py-2 rounded-lg border border-slate-200 shadow-sm w-full md:w-auto min-w-[200px]">
                              <div>
                                <div className="text-[11px] font-bold text-slate-700">{m.name || "Traveller"}</div>
                                <div className="text-[10px] font-medium text-slate-500 mt-0.5">{m.phone || "No phone"}</div>
                              </div>
                              {m.phone && (
                                <a href={`tel:${m.phone}`} onClick={e => e.stopPropagation()} className="ml-auto text-blue-600 hover:text-blue-700 p-1.5 bg-blue-50 hover:bg-blue-100 rounded-full transition-colors" title="Call passenger">
                                  <Smartphone className="w-3.5 h-3.5" />
                                </a>
                              )}
                            </div>
             ...
             {filtered.length === 0 ? (
                ...
                return (
                  <div key={bk.id} className="p-4 flex flex-col gap-3">
                    <div className="flex justify-between items-start cursor-pointer" onClick={() => toggleExpand(bk.id)}>
                      <div>
                        ...
                        <div className="text-slate-500 text-xs mt-1 flex items-center gap-2">
                          <span>{bk.phone || "—"}</span>
                          {bk.phone && (
                            <a href={`tel:${bk.phone}`} className="text-blue-500 hover:text-blue-600 p-1.5 bg-blue-50 rounded-full transition-colors" title="Call">
                              <Smartphone className="w-3.5 h-3.5" />
                            </a>
                          )}
                        </div>
                    ...
                    {expandedBookings.includes(bk.id) && bk.members && bk.members.length > 0 && (
                      <div className="mt-1 pt-3 border-t border-slate-100 flex flex-col gap-2">
                        {bk.members.map((m, i) => (
                          <div key={i} className="flex justify-between items-center bg-slate-50 p-2.5 rounded border border-slate-200 shadow-sm">
                            <div>
                              <div className="text-xs font-bold text-slate-700">{m.name || "Traveller"}</div>
                              <div className="text-[10px] font-medium text-slate-500 mt-0.5">{m.phone || "No phone"}</div>
                            </div>
                            {m.phone && (
                              <a href={`tel:${m.phone}`} onClick={e => e.stopPropagation()} className="text-blue-600 hover:text-blue-700 p-1.5 bg-blue-100 hover:bg-blue-200 rounded-full transition-colors" title="Call">
                                <Smartphone className="w-3.5 h-3.5" />
                              </a>
                            )}
                          </div>
