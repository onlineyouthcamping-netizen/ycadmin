...
import { 
  Users, ShieldCheck, UserCheck, PhoneCall, Award, Clock, Compass, Heart, MapPin, 
  Sparkles, Plus, Trash2, Copy, ArrowUp, ArrowDown, Eye, EyeOff, Edit2, Palette, Check
} from "lucide-react";
...
export const AboutTripCmsEditor: React.FC<AboutTripCmsEditorProps> = ({ data, onChange }) => {
  const cmsTitle = data?.title || "About This Trip";
  const cmsDescription = data?.description || "Get ready for an unforgettable journey through Northern India! Begin with a train journey from your city. Explore the cultural richness, mountain landscapes, and vibrant local traditions with expert guides.";
  const cards = (data?.cards && data.cards.length > 0) ? data.cards : DEFAULT_CARDS;

  const [editingCardId, setEditingCardId] = useState<string | null>(null);

  const updateData = (title: string, description: string, updatedCards: AboutTripCard[]) => {
  ...
  const handleUpdateCardField = (cardId: string, field: keyof AboutTripCard, value: any) => {
    ...
    <div className="space-y-6 text-xs">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="space-y-1">
            ...
            <Input 
              value={cmsTitle} 
              onChange={e => updateData(e.target.value, cmsDescription, cards)} 
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase text-slate-500">Trip Overview Description</label>
            <Textarea 
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs space-y-4">
        <div className="flex justify-between items-center border-b pb-3">
          <div>
            <h4 className="font-bold text-slate-800 text-xs uppercase tracking-wider">Highlight Badges & Cards ({cards.length})</h4>
            <p className="text-[10px] text-slate-400">Reorder, edit titles, change icons & customize theme colors</p>
          </div>
          <Button 
            type="button" 
            size="sm" 
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  card.isVisible ? "bg-white border-slate-200" : "bg-slate-50 border-slate-200/60 opacity-60",
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    >
                      {renderIcon(card.icon, "w-4 h-4", card.iconColor || "#ea580c")}
                    </div>
                    <div className="min-w-0">
                      <p className="font-bold text-slate-800 text-xs truncate">{card.title || "Untitled"}</p>
                      <p className="text-[10px] text-slate-400 truncate">{card.subtitle || "No subtitle"}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    >
                      {card.isVisible ? <Eye className="w-3.5 h-3.5 text-emerald-600" /> : <EyeOff className="w-3.5 h-3.5 text-slate-400" />}
                    </button>
                    <button
                      type="button"
                ...
                {isEditing && (
                  <div className="pt-3 border-t border-slate-100 space-y-3 bg-slate-50/70 p-3 rounded-lg mt-2">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold uppercase text-slate-400">Card Title</label>
                        <Input 
                          value={card.title} 
                      ...
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold uppercase text-slate-400">Subtitle</label>
                        <Input 
                          value={card.subtitle} 
                    ...
                    {/* Icon Selection */}
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold uppercase text-slate-400">Select Icon</label>
                        ...
                        {ICON_OPTIONS.map(opt => {
                          ...
                          return (
                            <button
                              key={opt.name}
                              type="button"
                              onClick={() => handleUpdateCardField(card.id, "icon", opt.name)}
                              className={cn(
                                "p-1.5 rounded-lg border text-xs flex items-center gap-1 transition-all",
                                isSel ? "bg-orange-50 border-orange-500 text-orange-600 font-bold" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
                              )}
                            >
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold uppercase text-slate-400">Icon Color</label>
                        <Input 
                          type="color" 
                          value={card.iconColor || "#ea580c"} 
                          onChange={e => handleUpdateCardField(card.id, "iconColor", e.target.value)} 
                          className="h-7 p-0.5 bg-white cursor-pointer"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold uppercase text-slate-400">Background</label>
                        <Input 
                          type="color" 
                          value={card.bgColor || "#fff7ed"} 
                          onChange={e => handleUpdateCardField(card.id, "bgColor", e.target.value)} 
                          className="h-7 p-0.5 bg-white cursor-pointer"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[9px] font-bold uppercase text-slate-400">Border</label>
                        <Input 
                          type="color" 
                          value={card.borderColor || "#ffedd5"} 
                          onChange={e => handleUpdateCardField(card.id, "borderColor", e.target.value)} 
                          className="h-7 p-0.5 bg-white cursor-pointer"
                        />
                      </div>
                    </div>
      ...
      <div className="bg-slate-900 text-white rounded-xl p-5 shadow-md space-y-4">
        <div className="flex justify-between items-center border-b border-slate-800 pb-2">
          ...
          <span className="text-[9px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded font-mono">Real-time CMS Output</span>
        </div>
        ...
        <div className="bg-white text-slate-900 rounded-xl p-6 space-y-5 shadow-xs">
          ...
          <p className="text-xs text-slate-600 leading-relaxed font-sans">{cmsDescription}</p>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-2">
            {cards.filter(c => c.isVisible).map(card => (
              <div 
                key={card.id}
              ...
              >
                <div className="p-2 rounded-lg shrink-0">
                  {renderIcon(card.icon, "w-5 h-5", card.iconColor || "#ea580c")}
                </div>
                <div>
