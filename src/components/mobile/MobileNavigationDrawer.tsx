...
export const MobileNavigationDrawer: React.FC<MobileNavigationDrawerProps> = ({
  ...
  const modules = [
    ...
    { title: "Expeditions & Trips", url: "/admin/trips", icon: Map },
    { title: "Accounting & Cash", url: "/admin/accounting", icon: DollarSign },
    { title: "Vendors Directory", url: "/admin/vendor-directory", icon: Building2 },
    { title: "Staff & HR", url: "/admin/hr?tab=staff", icon: Briefcase },
    { title: "Company Documents", url: "/admin/company-documents", icon: FileText },
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="w-[85%] sm:w-[320px] p-0 bg-slate-900 text-white border-r-0">
        {/* Profile Header */}
        <div className="p-4 border-b border-slate-800 bg-slate-950/50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs font-bold text-white truncate max-w-[150px]">{admin?.name || "Admin User"}</h3>
        ...
        <div className="p-3 space-y-1 overflow-y-auto max-h-[calc(100vh-140px)]">
          {modules.map((mod) => {
            const Icon = mod.icon;
            const isActive = location.pathname === mod.url || (mod.url !== "/admin" && location.pathname.startsWith(mod.url));
            return (
              <button
                key={mod.title}
                type="button"
