  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="admin-title">Footer Management</h1>
          <p className="admin-body">Customize the public website footer columns, contact details, social links, and visibility settings.</p>
        </div>
        <Button onClick={handleSave} disabled={saving} className="admin-button-primary bg-primary-orange hover:bg-primary-orange/90 text-white self-start sm:self-auto flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="admin-card border border-slate-200">
            <CardContent className="p-6 space-y-6">
              <div className="flex justify-between items-center border-b pb-4">
                <h2 className="admin-card-title flex items-center gap-2 text-lg">
                  <Layout className="w-5 h-5 text-primary-orange" /> Footer Link Columns
                </h2>
                <Button variant="outline" size="sm" onClick={addColumn} className="admin-button-outline text-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                {footerConfig.columns.map((col, colIdx) => (
                  <div key={col.id} className={`border rounded-2xl p-4 bg-slate-50/50 shadow-sm flex flex-col gap-4 relative ${!col.visible ? 'opacity-60 border-dashed' : 'border-slate-200'}`}>
                    ...
                    <div className="space-y-3 flex-1">
                      ...
                      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                        {col.links.map((link, linkIdx) => (
                          <div key={link.id} className={`flex items-center gap-2 bg-white p-2 rounded-xl border border-slate-200/80 ${!link.visible ? 'opacity-55 border-dashed' : ''}`}>
                ...
                {footerConfig.columns.length === 0 && (
                  <div className="md:col-span-2 text-center py-16 text-slate-400 border border-dashed rounded-2xl">
        ...
        <div className="space-y-6">
          ...
          <Card className="admin-card border border-slate-200">
            <CardContent className="p-6 space-y-6">
              <h2 className="admin-card-title flex items-center gap-2 text-base border-b pb-3">
                <Shield className="w-4 h-4 text-primary-orange" /> Visibility Controls
              </h2>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.field} className="flex items-center justify-between bg-slate-50/50 p-3 rounded-xl border border-slate-150">
          ...
          <Card className="admin-card border border-slate-200">
            <CardContent className="p-6 space-y-6">
              <div className="flex justify-between items-center border-b pb-3">
                <h2 className="admin-card-title flex items-center gap-2 text-base">
                  <Share2 className="w-4 h-4 text-primary-orange" /> Social Accounts
                </h2>
              ...
              <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
                {footerConfig.socialLinks.map((social, idx) => (
                  <div key={social.platform} className="space-y-1 bg-slate-50/50 p-3 rounded-xl border relative">
                    <div className="flex justify-between items-center">
          ...
          <Card className="admin-card border border-slate-200">
            <CardContent className="p-6 space-y-6">
              <h2 className="admin-card-title flex items-center gap-2 text-base border-b pb-3">
                <Globe className="w-4 h-4 text-primary-orange" /> Contact details & Tagline
              </h2>

              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label className="admin-label text-xs">Brand Name</Label>
                  <Input value={footerConfig.brandName} onChange={(e) => handleFieldChange('brandName', e.target.value)} className="admin-input h-9" placeholder="YOUTHCAMPING" />
                </div>
                <div className="space-y-1.5">
                  <Label className="admin-label text-xs">Support Phone</Label>
                  <Input value={footerConfig.phone} onChange={(e) => handleFieldChange('phone', e.target.value)} className="admin-input h-9" placeholder="+91-99242 46267" />
                </div>
                <div className="space-y-1.5">
                  <Label className="admin-label text-xs">Support Email</Label>
                  <Input value={footerConfig.email} onChange={(e) => handleFieldChange('email', e.target.value)} className="admin-input h-9" placeholder="info@youthcamping.com" />
                </div>
                <div className="space-y-1.5">
                  <Label className="admin-label text-xs">Website URL</Label>
                  <Input value={footerConfig.website} onChange={(e) => handleFieldChange('website', e.target.value)} className="admin-input h-9" placeholder="youthcamping.in" />
                </div>
                <div className="space-y-1.5">
                  <Label className="admin-label text-xs">Footer Logo URL</Label>
                  <Input value={footerConfig.logoUrl} onChange={(e) => handleFieldChange('logoUrl', e.target.value)} className="admin-input h-9" placeholder="/logo-stacked.png" />
                </div>
                <div className="space-y-1.5">
                  <Label className="admin-label text-xs">Copyright Text</Label>
                  <Input value={footerConfig.copyright} onChange={(e) => handleFieldChange('copyright', e.target.value)} className="admin-input h-9" placeholder="ALL RIGHTS RESERVED." />
                </div>
                <div className="space-y-1.5">
                  <Label className="admin-label text-xs">Office Address</Label>
                  <textarea value={footerConfig.address} onChange={(e) => handleFieldChange('address', e.target.value)} className="admin-input text-xs h-20 py-2" placeholder="Full office address..." />
                </div>
              </div>
            </CardContent>
          </Card>
