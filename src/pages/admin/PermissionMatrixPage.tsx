...
import { Grid, Search, RefreshCw, CheckCircle2, XCircle, ChevronDown, ChevronRight, Lock, Shield } from 'lucide-react';
import toast from 'react-hot-toast';

export default function PermissionMatrixPage() {
  const [matrixData, setMatrixData] = useState<{ roles: Array<{ id: string; name: string; isSystem: boolean }>; matrix: PermissionMatrixRow[] }>({
    roles: [],
    matrix: []
  });
  ...
  const fetchMatrix = async () => {
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to load permission matrix');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 text-white p-6 rounded-2xl shadow-lg border border-slate-800">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight">Enterprise Permission Matrix</h1>
          </div>
          <p className="text-slate-400 text-sm">
            Spreadsheet-style comparison matrix showing role permission assignments across every module.
          </p>
        </div>
        <Button onClick={fetchMatrix} variant="outline" className="bg-slate-800 text-slate-200 border-slate-700 hover:bg-slate-700">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto max-h-[75vh]">
          <Table className="border-collapse w-full text-xs">
            <TableHeader className="sticky top-0 z-20 bg-slate-900 text-white shadow-md">
              <TableRow className="border-b border-slate-800 hover:bg-slate-900">
                ...
                {matrixData.roles.map(role => (
                  <TableHead key={role.id} className="text-center font-bold text-slate-100 min-w-[120px] p-3 border-l border-slate-800">
            ...
            <TableBody>
              {Object.entries(groupedByModule).map(([moduleName, moduleRows]) => {
                    ...
                    <TableRow className="bg-slate-100/90 font-bold text-slate-800 hover:bg-slate-200/90 cursor-pointer transition-colors border-y border-slate-300">
                      ...
                      >
                        <div className="flex items-center gap-2 text-slate-900 font-bold text-xs uppercase tracking-wider">
                          ...
                          <span>{moduleName} ({moduleRows.length})</span>
                        </div>
                      </TableCell>
                    </TableRow>

                    {/* Permission Rows */}
                    {isExpanded && moduleRows.map((permRow, idx) => (
                      <TableRow key={permRow.permissionId} className={`hover:bg-amber-50/50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                        <TableCell className="font-medium p-3 sticky left-0 bg-white z-10 border-r border-slate-200 shadow-r">
                          <div className="space-y-0.5">
                            <span className="font-bold text-slate-900 block">{permRow.name}</span>
                            <span className="text-[10px] text-slate-500 font-mono block">{permRow.action}</span>
                          </div>
                        </TableCell>
                        {matrixData.roles.map(role => {
                          const isGranted = permRow.roles[role.id] ?? false;
                          return (
                            <TableCell key={role.id} className="text-center p-3 border-l border-slate-200">
                              {isGranted ? (
