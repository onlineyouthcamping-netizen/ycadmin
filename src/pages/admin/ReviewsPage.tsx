...
import { Plus, Pencil, Trash2, Star, MessageSquare, MapPin, X, Sparkles, Filter, CheckCircle2 } from "lucide-react";
...
export default function ReviewsPage() {
  ...
  const toggleFeatured = async (r: any) => {
    try {
      ...
      toast.success(r.isFeatured ? "Unpublished from featured" : "Featured on website");
      load();
    } catch (error) {
  ...
  const filteredReviews = useMemo(() => {
    return reviews.filter(r => {
      const matchTrip = selectedTripFilter === "all" || r.tripId === selectedTripFilter || r.tripName === selectedTripFilter;
      const q = searchQuery.toLowerCase();
      const matchSearch = !q || (r.userName || "").toLowerCase().includes(q) || (r.city || "").toLowerCase().includes(q) || (r.comment || "").toLowerCase().includes(q) || (r.tripName || "").toLowerCase().includes(q);
  ...
  const stats = useMemo(() => {
    ...
    const avgRating = total > 0 ? (reviews.reduce((acc, r) => acc + (Number(r.rating) || 5), 0) / total).toFixed(1) : "5.0";
  ...
  return (
    <div className="space-y-6 animate-fade-in p-6 bg-slate-50/50 min-h-screen -mx-6 -my-6">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-extrabold text-slate-900 tracking-tight font-montserrat">Customer Reviews Hub</h1>
              <span className="bg-emerald-100 text-emerald-800 font-extrabold text-[9px] px-2 py-0.5 rounded-full uppercase">
                Synced Live
              </span>
            </div>
            <p className="text-xs text-slate-500 font-medium mt-0.5">Manage customer testimonials, trip photos, and website featured reviews</p>
          </div>
        </div>

        <Button onClick={openCreate} className="h-10 px-5 rounded-xl font-extrabold text-xs bg-[#FF5400] hover:bg-[#d44500] text-white flex items-center gap-2 shadow-md hover:shadow-lg transition-all shrink-0">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Featured on Web</p>
            <p className="text-2xl font-black text-emerald-600 mt-1">{stats.featured}</p>
          </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Trips Covered</p>
            <p className="text-2xl font-black text-purple-600 mt-1">{stats.uniqueTrips}</p>
          </div>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-2xs space-y-3">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar w-full sm:w-auto pb-1 sm:pb-0">
            ...
            {trips.map(t => {
              const count = reviews.filter(r => r.tripId === (t.id || t._id) || r.tripName === t.title).length;
              return (
                <button
                  key={t.id || t._id}
      ...
      {loading ? (
        <div className="text-center py-20 bg-white border border-slate-200/80 rounded-2xl">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest animate-pulse">Loading customer reviews...</p>
        </div>
        ...
        <div className="text-center py-16 bg-white border border-slate-200/80 rounded-2xl space-y-3">
          ...
          <p className="text-sm font-extrabold text-slate-700">No Reviews Match Current Filter</p>
          <p className="text-xs text-slate-400">Click "Add New Review" above to publish traveler feedback for this trip.</p>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`bg-white border rounded-2xl p-5 shadow-2xs hover:shadow-md transition-all flex flex-col justify-between space-y-4 relative group ${
                  r.isFeatured ? "border-emerald-200 bg-emerald-50/20" : "border-slate-200/80"
                }`}
              >
                <div>
                  {/* Top Header Row */}
                  <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
                    <div className="flex items-center gap-3">
                      {r.userImage ? (
                        <img src={r.userImage} alt={r.userName} className="w-10 h-10 rounded-full object-cover border border-slate-200 shrink-0" />
                      ) : (
                      ...
                      <div className="min-w-0">
                        <h4 className="font-extrabold text-sm text-slate-900 truncate">{r.userName}</h4>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="text-[10px] font-bold text-slate-500 truncate">{r.tripName || "Trip Review"}</span>
                        </div>
                      </div>
                    </div>

                    <div className="flex items-center gap-1 shrink-0">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(r)} className="h-8 w-8 text-slate-500 hover:text-slate-900 rounded-lg">
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(r.id || r._id)} className="h-8 w-8 text-rose-500 hover:bg-rose-50 rounded-lg">
                  ...
                  <div className="pt-3 space-y-2">
                    <div className="flex items-center gap-0.5 text-amber-400">
                      ...
                      <span className="text-xs font-bold text-slate-700 ml-1.5">{ratingCount}.0</span>
                    </div>
                    ...
                    {((r.photos && r.photos.length > 0) || r.photo) && (
                      <div className="pt-2 grid grid-cols-3 gap-1.5">
                        {((r.photos && r.photos.length > 0) ? r.photos.slice(0, 3) : [r.photo]).map((pUrl: string, pIdx: number) => (
                          <div key={pIdx} className="aspect-[4/3] bg-slate-100 rounded-lg overflow-hidden border border-slate-200/60">
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${r.isFeatured ? "text-emerald-700" : "text-slate-400"}`}>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-2xl w-[95vw] sm:w-full rounded-2xl p-6 border border-slate-200 shadow-xl max-h-[92dvh] overflow-y-auto custom-scrollbar">
          ...
          <div className="grid gap-5 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="space-y-1.5">
                 ...
                 <Input className="rounded-xl h-10 text-xs font-bold" value={formData.userName} onChange={(e) => setFormData({...formData, userName: e.target.value})} placeholder="e.g. Bhumit Rabadiya" />
               </div>
               <div className="space-y-1.5">
                 <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Customer City / State</Label>
                 <Input className="rounded-xl h-10 text-xs font-bold" value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} placeholder="e.g. Ahmedabad, Gujarat" />
               </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div className="space-y-1.5">
                 <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Map to Trip *</Label>
                 <Select 
               ...
               <div className="space-y-1.5">
                 <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Star Rating (1 - 5 Stars)</Label>
                 <Select 
            ...
            <div className="space-y-1.5">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Review Comment / Feedback *</Label>
              <Textarea className="rounded-xl min-h-[100px] p-3 text-xs font-medium leading-relaxed" value={formData.comment} onChange={(e) => setFormData({...formData, comment: e.target.value})} placeholder="Paste traveler's feedback story..." />
            </div>
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-4">
              <h4 className="text-xs font-extrabold text-slate-800 uppercase tracking-wider font-montserrat flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-[#FF5400]" /> Review Photos &amp; Media Uploaders
              </h4>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2 bg-white p-3.5 rounded-xl border border-slate-200">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-slate-700">1. Customer Profile Avatar</Label>
                  <p className="text-[10px] text-slate-400">Small face avatar photo displayed next to name</p>
                  <ImageUpload 
                ...
                <div className="space-y-2 bg-white p-3.5 rounded-xl border border-slate-200">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-slate-700">2. Main Cover Photo</Label>
                  <p className="text-[10px] text-slate-400">Landscape hero photo at top of card</p>
                  <ImageUpload 
              ...
              <div className="space-y-2.5 bg-white p-4 rounded-xl border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-700">3. Multiple Trip Gallery Photos</Label>
                    <p className="text-[10px] text-slate-400">Upload multiple photos taken by customer during the trip</p>
                  </div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200 mt-1">
               <div>
                 <p className="text-xs font-bold text-slate-800">Publish &amp; Feature on Website</p>
                 <p className="text-[10px] text-slate-400">Display this review in the "WHAT TRAVELERS SAY" section on website</p>
               </div>
          ...
          <DialogFooter className="pt-4 border-t flex flex-col sm:flex-row gap-2">
            <Button variant="outline" onClick={() => setModalOpen(false)} className="w-full sm:w-auto rounded-xl h-9 px-5 font-extrabold text-xs">Cancel</Button>
            <Button onClick={handleSave} className="w-full sm:w-auto rounded-xl h-9 px-6 font-extrabold text-xs bg-[#FF5400] hover:bg-[#d44500] text-white shadow-md">Save Review</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
