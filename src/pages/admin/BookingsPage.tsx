    ...
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-[#F4F7FB] text-[#162B45] font-sans antialiased -mx-6 -my-6">
      ...
      <div className="flex items-center justify-between border-b border-[#E2E8F0] bg-white px-6 h-14 font-sans shadow-xs">
        <div className="flex items-center gap-4 flex-1">
          <div className="font-extrabold text-slate-800 text-sm tracking-tight">Bookings</div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="h-8 px-3.5 border border-slate-200 hover:bg-slate-50 rounded-[6px] text-xs text-slate-700 font-semibold flex items-center gap-1.5 transition-colors cursor-pointer bg-white" onClick={() => setShowTrips(true)} title="Trip Manager">
          ...
          return (
            ...
            >
              ...
              <span className={cn(
                ...
                isActive ? "bg-white text-[#0F172A]" : "bg-slate-100 text-slate-800"
              )}>
                {chip.count}
              </span>
            </button>
            ...
            <select
              ...
              onChange={e => {
                ...
                if (val === 'all') {
                  ...
                  setSelectedStatuses(['Unconfirmed']);
                  setQuickFilter('unconfirmed_bookings');
                } else if (val === 'Confirmed') {
                  setSelectedStatuses(['Confirmed']);
                  setQuickFilter('confirmed_bookings');
                } else if (val === 'Cancelled') {
                  setSelectedStatuses(['Cancelled']);
                  setQuickFilter('cancelled_bookings');
                } else if (val === 'Completed') {
                  setSelectedStatuses(['Completed']);
                  setQuickFilter('completed_bookings');
                }
              }}
              className="w-full h-8 px-2 bg-white border border-slate-200 rounded text-xs outline-none"
            >
